...
echo "[riscv-systemc-mmio] Preparing Docker build volume"
"${docker_cmd}" run --rm \
  -v "${build_volume}:/docker-build" \
  "${image_name}" \
  sh -c "mkdir -p /docker-build/riscv && chown $(id -u):$(id -g) /docker-build /docker-build/riscv"

echo "[riscv-systemc-mmio] Building in Docker image ${image_name}"
"${docker_cmd}" run --rm \
  -u "$(id -u):$(id -g)" \
  -v "${repo_root}:/work:ro" \
  -v "${build_volume}:/docker-build" \
  ...
  -w /work \
  "${image_name}" \
  sh -lc 'cmake -S /work -B /docker-build/riscv -G Ninja && cmake --build /docker-build/riscv --target harbor_riscv_systemc_mmio && mkdir -p /host-build/examples/riscv/minimal && cp /docker-build/riscv/examples/riscv/minimal/riscv-systemc-mmio.elf /docker-build/riscv/examples/riscv/minimal/riscv-systemc-mmio.map /host-build/examples/riscv/minimal/'
